...
| Container | `app/components/adminit/dashboard/tabbed_container_component.rb` | Wraps the widgets of one resource in a card: resource title, and one tab per widget (tabs are always rendered, even for a single widget), each carrying its own derived "view all" link. |
...
| `view_all_params` | `nil` | Optional query params merged into this widget's own "view all" link (e.g. `{assignee: "me"}`). |
...
4. The container renders the resource title (`adminit.navigation.<resource>s`) above the tabs. Each
   tab panel gets its own **"view all"** link, derived from that widget's `view_all_params` (defaults
   to the resource's adminit index route, `adminit_#{resource.to_s.pluralize}_path`, with no params).
   Per-tab (not per-card) so switching tabs never shows a stale link from a different widget.
...
Pass `id_prefix:` explicitly for any tabs group that renders inside a Turbo-morphed page (the adminit
layout sets `turbo_refreshes_with method: :morph`). Idiomorph matches nodes by `id`, so a prefix
derived from `object_id` or any other per-render value gets a fresh id on every morph and idiomorph
replaces the tab/panel subtree instead of patching it — resetting whichever tab the user had open.
`TabbedContainerComponent` derives a stable prefix from `resource` (`tab_id_prefix`); reuse that
pattern for any other caller.
