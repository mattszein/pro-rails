...
RSpec.describe Support::MessagesController, type: :controller do
  ...
  describe "POST #create" do
    let(:valid_params) { {ticket_id: ticket.id, message: {content: "Hello world"}} }
    let(:invalid_params) { {ticket_id: ticket.id, message: {content: ""}} }
    ...
    context "when logged in as ticket creator" do
      ...
      context "with valid parameters" do
        ...
        it "is authorized" do
          expect { subject }.to be_authorized_to(:create?, an_instance_of(Message)).with(MessagePolicy).with_context(user: creator_account)
        end
        ...
        it "assigns the message to the current account" do
          subject
          expect(Message.last.account_id).to eq(creator_account.id)
        end
    ...
    context "when logged in as ticket assignee (admin)" do
      ...
      context "with valid parameters" do
        ...
        it "is authorized" do
          expect { subject }.to be_authorized_to(:create?, an_instance_of(Message)).with(MessagePolicy).with_context(user: assignee_account)
        end
        ...
        it "assigns the message to the assignee account" do
          subject
          expect(Message.last.account_id).to eq(assignee_account.id)
        end
    ...
    context "when assignee is different from creator" do
      ...
      context "assignee can send messages on other users' tickets" do
        ...
        it "is authorized" do
          expect { subject }.to be_authorized_to(:create?, an_instance_of(Message)).with(MessagePolicy).with_context(user: different_assignee)
        end
        ...
        it "assigns the message to the assignee" do
          subject
          expect(Message.last.account_id).to eq(different_assignee.id)
        end
      end
    end
    ...
    context "when ticket does not exist" do
      ...
      let(:params) { {ticket_id: 999999, message: {content: "Hello world"}} }
      ...
      it "sets ticket not found flash alert" do
        subject
        expect(flash.now[:alert]).to eq(I18n.t("errors.ticket_not_found"))
      end
