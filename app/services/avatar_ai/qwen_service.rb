...
module AvatarAi
  ...
  # The DashScope multimodal API uses a chat-style payload format,
  # not the standard image generation format, so ruby_llm cannot wrap it.
  class QwenService
    ...
    # Returns {io:, content_type:} for the downloaded image.
    ...
    def request_image_url(prompt, size:)
      ...
      unless response.is_a?(Net::HTTPSuccess) && parsed.dig("output", "choices")
        raise AvatarAi::ImageGenerator::GenerationError,
          "Qwen API error: #{parsed["message"] || parsed.inspect}"
      end

      image_url = parsed.dig("output", "choices", 0, "message", "content", 0, "image")
      raise AvatarAi::ImageGenerator::GenerationError, "Qwen returned no image URL" unless image_url
    ...
    def download_image(url)
      ...
      raise AvatarAi::ImageGenerator::GenerationError, "Failed to download Qwen image: #{e.message}"
    end
